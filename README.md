...
# 布署运行
   - 将frps[^ckwx4](about:config "frp的server端")运行布署在一台低延迟高带宽**具有公网ip**的服务器(A)上

   - 将frpc[^ckwx5](about:config "frp的client端")运行布署在一台网速足够快并且**[能够运行识别程序](about:config "目前的python代码需要有gpu以及不弱的cpu")**的计算机(B)上, frpc会在A机器上注册服务, 将对 https://adl.seafishery.com 的请求(本质上是对A服务器443端口的https请求)转发到B机器的本地端口20778
